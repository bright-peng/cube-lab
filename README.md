...
一个零运行时依赖、可离线使用的三阶魔方网页：真实三维几何投影、同步展开图、同心环编码视图，以及从当前状态计算的七阶段公式求解。
...
- 2D：十字展开图和六个同心环，与 3D 共用同一状态；色块选中高亮联动，可放大查看。
...
从内到外分别是 U、R、F、D、L、B。每圈九个区段，从上方开始顺时针排列，对应该面的九宫格行优先编号 1–9。

这是一种可逆的编码布局，**不是标准展开图，也不是保持实际邻接关系的三维投影**。用户提供的是同心圆的文字描述，没有可用参考图片，因此这里是一个明确标注含义的原创原型，而不是对参考图的复刻。
...
src/renderer.js               透视 3D / SVG 展开图 / 同心环
...
37 项 Node 测试通过，其中包含 200 组独立打乱求解；35 项离线 Chromium 交互检查通过。浏览器检查为可选开发工具，需要 Python 的 Playwright 和 Chromium：
...
每次推送、Pull Request 或手动运行 **Test and build Cube Lab**，都会执行语法检查、37 项核心测试和单文件构建。成功后可在 Actions 运行页面下载 `cube-lab-offline`，解压得到 `index.html`；产物保留 14 天。此流程不需要启用 Pages，也不需要额外配置密钥。
